...
const parseClassElement = (
	...
	return {
		...
		disjointWith: (selectors.disjoinWith($, $el) || []).map(l =>
			applyBaseUrlToLink(baseUrl, l)
		),
...
const parseProperty = (
	...
	return {
		...
		subPropertyOf: applyBaseUrlToLink(baseUrl, selectors.subPropertyOf($, $el))
	};
};
	...
	const parsedVocab = {
		...
		sections: {
			...
			activityTypes: new Ontology<ParsedClass>({
				members: parseClassTable(
					$,
					$("#h-activity-types ~ table > tbody"),
					baseUrl
				)
			}),
